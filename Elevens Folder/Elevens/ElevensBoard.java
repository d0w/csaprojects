    ...
    private boolean containsPairSum11(List<Integer> selectedCards) {
        for (int i = 0; i < selectedCards.size() - 1; i++) {
            for (int j = i+1; j < selectedCards.size(); j++) {
                ...
                cardAt(selectedCards.get(j)).getPoint() == 11) {
                    return true;
                }
            }
        }
        ...
        for (int i = 0; i < selectedCards.size() - 2; i++) {
            for (int j = i+1; j < selectedCards.size() - 1; j++) {
                for (int k = j + 1; k < selectedCards.size(); k++) {
                    if (cardAt(selectedCards.get(i)).getPoint() == 0 && 
                    cardAt(selectedCards.get(j)).getPoint() == 0 &&
                    cardAt(selectedCards.get(k)).getPoint() == 0) {
                        return true;
                    }
                }
            }
